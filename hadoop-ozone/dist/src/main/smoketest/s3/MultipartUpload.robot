...
#upload parts
    ...
    ${result} =         Execute AWSS3APICli     upload-part --bucket ${BUCKET} --key multipartKey3 --part-number 3 --body /tmp/part2 --upload-id ${uploadID}
    ${eTag2} =          Execute and checkrc     echo '${result}' | jq -r '.ETag'   0
                        Should contain          ${result}    ETag
...
#complete multipart upload(merge with few parts)
    ${result} =         Execute AWSS3APICli     complete-multipart-upload --upload-id ${uploadID} --bucket ${BUCKET} --key multipartKey1 --multipart-upload 'Parts=[{ETag=${eTag1},PartNumber=1},{ETag=${eTag3},PartNumber=3}]'
                        Should contain          ${result}    ${BUCKET}
                        Should contain          ${result}    multipartKey1
                        Should contain          ${result}    ETag

    ${result} =                 Execute AWSS3ApiCli        get-object --bucket ${BUCKET} --key multipartKey1 /tmp/multipartKey1.result
                                Execute                    cat /tmp/part1 /tmp/part3 >> /tmp/multipartKey1
    Compare files               /tmp/multipartKey1         /tmp/multipartKey1.result
